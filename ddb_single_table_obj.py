...
# see: https://github.com/boto/boto3/issues/665    
    ...
    def save_db_record(self, pk, sk, data, **items):
        if data == "":
            data = " "
    ...
    def get_db_records_by_secondary_key(self, sk, data_condition=None):
        params = {
            "IndexName": "gsi_1",
            "KeyConditionExpression": Key("sk").eq(sk)
        }
        if data_condition is not None:
            params ["KeyConditionExpression"] &= Key("data").eq(data_condition)
        try:
    ...
    """
    def query_db_record(self, pk, data_condition=None):
        try:
